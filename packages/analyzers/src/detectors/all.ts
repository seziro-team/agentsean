...
const FACET_PARAMS =
  /[?&](sort|order|orderby|view|display|limit|per_page|filter|color|size|price)=/i;
...
export function detectResp(ctx: AuditContext): FindingDraft[] {
  ...
  push(
    ...
    pages
      .filter(
        (p) => p.isInternal && inRange(p.statusCode, 400, 499) && p.statusCode !== 429,
      )
      .map((p) => p.url),
    {},
  );
  push(
    out,
    "RESP.429",
    pages.filter((p) => p.statusCode === 429).map((p) => p.url),
    {},
  );
  push(
    out,
    "RESP.NO_RESPONSE",
    pages.filter((p) => p.statusCode === null).map((p) => p.url),
    {},
  );
  push(
    out,
    "RESP.REDIRECT_CHAIN",
    pages.filter((p) => hops(p) >= 2).map((p) => p.url),
    {},
  );
  push(
    out,
    "RESP.REDIRECT_CHAIN_LONG",
    pages.filter((p) => hops(p) >= 5).map((p) => p.url),
    {},
  );
  push(
    out,
    "RESP.REDIRECT_LOOP",
    pages.filter((p) => p.redirectLoop).map((p) => p.url),
    {},
  );
  ...
  push(
    out,
    "RESP.302_PERMANENT",
    pages.filter((p) => p.statusCode === 302 || p.statusCode === 307).map((p) => p.url),
    {},
    0.5,
  );
  push(
  ...
  push(
    out,
    "RESP.HTTP_REFRESH",
    pages.filter((p) => Boolean(p.headers.refresh)).map((p) => p.url),
    {},
  );
  push(
    out,
  ...
  push(
    out,
    "RESP.REDIRECT_TO_NOINDEX",
    pages.filter((p) => hops(p) >= 1 && noindexOf(p)).map((p) => p.url),
    {},
  );
  ...
  if (toHome.length >= 20)
    push(
      out,
      "RESP.SOFT_REDIRECT_HOME",
      toHome.map((p) => p.url),
      { count: toHome.length },
    );
  push(
    out,
    "RESP.MIXED_PROTOCOL_CHAIN",
    pages
      .filter((p) => {
  ...
  push(
    out,
    "RESP.SLOW_TTFB",
    pages.filter((p) => p.ttfbMs > 600).map((p) => p.url),
    {},
  );
  push(
    out,
    "RESP.NO_304",
    pages.filter((p) => p.supports304 === false).map((p) => p.url),
    {},
  );
  push(
    out,
    "RESP.BAD_CONTENT_TYPE",
    pages
      .filter(
        (p) =>
          p.html !== null && p.contentType !== "" && !/html|xml/i.test(p.contentType),
      )
      .map((p) => p.url),
    {},
  );
  ...
  push(
    out,
    "RESP.OVER_15MB",
    pages.filter((p) => p.decodedBytes > 15_728_640).map((p) => p.url),
    {},
  );
  return out;
}
...
export function detectCanon(ctx: AuditContext): FindingDraft[] {
  ...
  push(
    out,
    "CANON.MULTIPLE_SAME",
    pages.filter((p) => (p.extract?.canonicalHtml.length ?? 0) > 1).map((p) => p.url),
    {},
  );
  push(
    out,
  ...
  push(
    out,
    "CANON.OUTSIDE_HEAD",
    pages.filter((p) => p.extract?.canonicalOutsideHead).map((p) => p.url),
    {},
  );
  push(
    out,
    "CANON.HEAD_BROKEN_EARLY",
    pages.filter((p) => p.extract?.headBrokenEarly).map((p) => p.url),
    {},
  );
  push(
    out,
    ...
    pages
      .filter(
        (p) =>
          (p.extract?.canonicalHtml[0] ?? "").length > 0 &&
  ...
  push(
    out,
    "CANON.EMPTY_MALFORMED",
    pages
      .filter((p) =>
        p.extract?.canonicalHtml.some((c) => !c.trim() || !absMaybe(c, p.finalUrl)),
      )
      .map((p) => p.url),
    {},
  );
  ...
  push(
    out,
    "CANON.CANONICALISED",
    pages.filter((p) => absCanonical(p) && !selfCanonical(p)).map((p) => p.url),
    {},
  );
  push(
    out,
    "CANON.CANONICALISED_IN_SITEMAP",
    pages
  ...
  push(
    ...
    pages
      .filter(
        (p) => /[?&](utm_|gclid|fbclid|msclkid|_ga)/i.test(p.url) && selfCanonical(p),
      )
      .map((p) => p.url),
    {},
  );
...
export function detectRobots(ctx: AuditContext): FindingDraft[] {
  ...
  const blockedCss = htmlPages(ctx).filter(
    (p) =>
      (p.extract?.stylesheets.some((s) => s.absUrl && !pageAllowed(ctx, s.absUrl)) ??
        false) ||
      (p.extract?.scripts.some((s) => s.absUrl && !pageAllowed(ctx, s.absUrl)) ??
        false),
  );
  push(
    out,
    "ROBOTS.BLOCKS_CSS_JS",
    blockedCss.map((p) => p.url),
    {},
  );
  push(
    out,
    "ROBOTS.BLOCKS_IMAGES",
    htmlPages(ctx)
      .filter((p) =>
        p.extract?.images.some((i) => i.absUrl && !pageAllowed(ctx, i.absUrl)),
      )
      .map((p) => p.url),
    {},
  );
...
export function detectDirect(ctx: AuditContext): FindingDraft[] {
  ...
  if (hasReq(ctx, "gsc") && ctx.gsc?.clicksByUrl) {
    push(
      out,
      "DIRECT.NOINDEX_INDEXABLE_TEMPLATE",
      pages
  ...
  push(
    out,
    "DIRECT.NOINDEX_UNEXPECTED",
    pages.filter((p) => noindexOf(p) && sitemap.has(p.url)).map((p) => p.url),
    {},
  );
  push(
    out,
    "DIRECT.NOINDEX_HTML_HEADER_MISMATCH",
    pages
      .filter((p) => {
  ...
  push(
    out,
    "DIRECT.MULTIPLE_NOINDEX",
    pages
      .filter(
        (p) =>
          (p.extract?.robotsMeta.filter((m) => /noindex/i.test(m)).length ?? 0) > 1,
      )
      .map((p) => p.url),
    {},
  );
  push(
    ...
    pages
      .filter(
        (p) => (p.extract?.robotsMeta.length ?? 0) > 0 && !p.extract?.robotsMetaInHead,
      )
      .map((p) => p.url),
    {},
  );
  ...
  push(
    out,
    "DIRECT.NOFOLLOW_SITEWIDE",
    pages
      .filter(
        (p) => nofollowOf(p) && (p.extract?.links.some((l) => l.isInternal) ?? false),
      )
      .map((p) => p.url),
    {},
  );
  push(
    out,
    "DIRECT.NONE",
    pages.filter((p) => robotsTokens(p).includes("none")).map((p) => p.url),
    {},
  );
  push(
    out,
    "DIRECT.UNAVAILABLE_AFTER_PAST",
    pages
  ...
  if (hasReq(ctx, "gsc")) {
    push(
      out,
      "DIRECT.NOSNIPPET_ON_MONEY_PAGE",
      pages
        .filter(
          (p) =>
            robotsTokens(p).includes("nosnippet") &&
  ...
  push(
    out,
    "DIRECT.MAX_SNIPPET_ZERO",
    pages.filter((p) => robotsTokens(p).includes("max-snippet:0")).map((p) => p.url),
    {},
  );
  push(
    out,
    "DIRECT.NO_MAX_IMAGE_PREVIEW",
    pages
      .filter(
        (p) =>
          p.extract?.jsonLd.some((j) =>
            /Article|Product/i.test(JSON.stringify(j.parsed)),
          ) && !robotsTokens(p).some((t) => t.includes("max-image-preview")),
      )
      .map((p) => p.url),
    {},
  );
  push(
    out,
    "DIRECT.NOIMAGEINDEX",
    pages.filter((p) => robotsTokens(p).includes("noimageindex")).map((p) => p.url),
    {},
  );
  const known = new Set([
    "index",
  ...
  push(
    out,
    "DIRECT.DISALLOW_MASKS_NOINDEX",
    pages.filter((p) => !p.robotsAllowed && noindexOf(p)).map((p) => p.url),
    {},
  );
  return out;
}
...
export function detectSmap(ctx: AuditContext): FindingDraft[] {
  ...
  for (const sm of sms) {
    ...
    const badLast = sm.urls.filter(
      (u) => u.lastmod && !/^\d{4}-\d{2}-\d{2}/.test(u.lastmod),
    );
    if (badLast.length) push(out, "SMAP.LASTMOD_INVALID_FORMAT", [sm.url], {});
    const future = sm.urls.filter(
      (u) => u.lastmod && Date.parse(u.lastmod) > Date.now(),
    );
  ...
  for (const sm of sms)
    for (const u of sm.urls) counts.set(u.loc, (counts.get(u.loc) ?? 0) + 1);
  const dups = [...counts.entries()].filter(([, n]) => n > 1).map(([u]) => u);
  push(out, "SMAP.DUP_ACROSS_SITEMAPS", dups, {});
  if (
    sms.some((s) => s.statusCode === 200) &&
  ...
  const imageHeavy =
    htmlPages(ctx).filter((p) => (p.extract?.images.length ?? 0) > 5).length >
...
export function detectCrawl(ctx: AuditContext): FindingDraft[] {
  ...
  push(
    out,
    "CRAWL.DEPTH_GT_3",
    pages.filter((p) => p.depth > 3).map((p) => p.url),
    {},
  );
  push(
    out,
    "CRAWL.DEPTH_GT_5",
    pages.filter((p) => p.depth > 5).map((p) => p.url),
    {},
  );
  push(
    out,
  ...
  push(
    out,
    "CRAWL.CALENDAR_TRAP",
    pages.filter((p) => DATE_RE.test(p.url)).map((p) => p.url),
    {},
  );
  push(
    out,
    "CRAWL.SESSION_ID",
    pages.filter((p) => SESSION_RE.test(p.url)).map((p) => p.url),
    {},
  );
  push(
    out,
    "CRAWL.FACET_EXPLOSION",
    pages.filter((p) => (p.url.match(/[?&]/g)?.length ?? 0) >= 3).map((p) => p.url),
    {},
  );
  push(
    out,
    "CRAWL.SORT_PARAMS",
    pages.filter((p) => FACET_PARAMS.test(p.url)).map((p) => p.url),
    {},
  );
  ...
  if (hasReq(ctx, "gsc")) {
    push(
      out,
      "CRAWL.DISCOVERED_NOT_INDEXED",
      Object.entries(ctx.gsc?.coverageStateByUrl ?? {})
    ...
    push(
      out,
      "CRAWL.STALE_IMPORTANT",
      Object.entries(ctx.gsc?.clicksByUrl ?? {})
        .filter(([, c]) => c > 0)
        .map(([u]) => u)
        .filter(() => false),
      {},
...
export function detectLink(ctx: AuditContext): FindingDraft[] {
  ...
  push(
    ...
    pages
      .filter(
        (p) =>
          (p.extract?.links.filter((l) => l.isInternal).length ?? 0) === 0 &&
          p.statusCode === 200,
      )
      .map((p) => p.url),
    {},
  ...
  push(
    ...
    pages
      .filter((p) =>
        p.extract?.links.some((l) => GENERIC_ANCHORS.has(l.anchorText.toLowerCase())),
      )
      .map((p) => p.url),
    {},
  );
  ...
  push(
    out,
    "LINK.BROKEN_FRAGMENT",
    pages
      .filter((p) =>
        p.extract?.links.some((l) => l.href.includes("#") && l.href.split("#")[1]),
      )
      .map((p) => p.url),
    {},
    0.4,
  ...
  push(
    ...
    pages
      .filter((p) =>
        p.extract?.links.some((l) => /localhost|127\.0\.0\.1|\.local/i.test(l.absUrl)),
      )
      .map((p) => p.url),
    {},
  );
  push(
    out,
    "LINK.UNSAFE_CROSS_ORIGIN",
    pages.filter((p) => (p.extract?.unsafeTargetBlanks ?? 0) > 0).map((p) => p.url),
    {},
  );
  push(
    out,
    "LINK.PROTOCOL_RELATIVE",
    pages
...
export function detectPage(ctx: AuditContext): FindingDraft[] {
  const out: FindingDraft[] = [];
  const pages = htmlPages(ctx);
  const paginated = pages.filter(
    (p) => /[?&]page=\d+/i.test(p.url) || /\/page\/\d+/i.test(p.url),
  );
  push(
    out,
    "PAGE.CANON_TO_P1",
    paginated
      .filter((p) => {
        const c = absCanonical(p);
        return Boolean(
          c && /page=1\b|\/page\/1\b/i.test(c) && !/page=1\b|\/page\/1\b/i.test(p.url),
        );
      })
      .map((p) => p.url),
    {},
  ...
  push(
    out,
    "PAGE.NON_200",
    paginated.filter((p) => p.statusCode !== 200).map((p) => p.url),
    {},
  );
  push(
    out,
    "PAGE.UNLINKED",
    paginated.filter((p) => p.inlinkCount === 0).map((p) => p.url),
    {},
  );
  push(
    out,
    "PAGE.LOOP",
    paginated.filter((p) => p.extract?.paginationRel.next === p.url).map((p) => p.url),
    {},
  );
  ...
  push(
    out,
    "PAGE.NOINDEX",
    paginated.filter(noindexOf).map((p) => p.url),
    {},
  );
  push(
    out,
    "PAGE.INFINITE_SCROLL_NO_URLS",
    pages
  ...
  push(
    out,
    "PAGE.DUPLICATE_TITLES",
    [...titles.values()].filter((v) => v.length > 1).flat(),
    {},
  );
...
export function detectParam(ctx: AuditContext): FindingDraft[] {
  ...
  push(
    out,
    "PARAM.TRACKING_INDEXABLE",
    pages
      .filter(
        (p) => /[?&](utm_|gclid|fbclid|msclkid|_ga)/i.test(p.url) && isIndexable(p),
      )
      .map((p) => p.url),
    {},
  );
  ...
  push(
    out,
    "PARAM.FACET_ZERO_RESULTS",
    htmlPages(ctx)
      .filter(
        (p) =>
          /no results|0 products|nothing found/i.test(p.extract?.mainText ?? "") &&
          p.statusCode === 200,
      )
      .map((p) => p.url),
    {},
  ...
  push(
    out,
    "PARAM.FACET_IN_SITEMAP",
    pages.filter((p) => FACET_PARAMS.test(p.url) && sm.has(p.url)).map((p) => p.url),
    {},
  );
  push(
    out,
...
export function detectDup(ctx: AuditContext): FindingDraft[] {
  ...
  push(
    out,
    "DUP.TITLE",
    dupField(pages, (p) => p.extract?.title ?? ""),
    {},
  );
  push(
    out,
    "DUP.META_DESC",
    dupField(pages, (p) => p.extract?.metaDescription ?? ""),
    {},
  );
  push(
    out,
  ...
  push(
    out,
    "DUP.BOILERPLATE_RATIO",
    pages.filter((p) => (p.extract?.boilerplateRatio ?? 0) > 0.75).map((p) => p.url),
    {},
  );
  ...
  push(
    out,
    "DUP.PRINT_VERSION",
    pages.filter((p) => /[?&]print=|\/print\//i.test(p.url)).map((p) => p.url),
    {},
  );
  if (
    ctx.crawl.originProbe.https &&
...
export function detectThin(ctx: AuditContext): FindingDraft[] {
  ...
  push(
    out,
    "THIN.LOW_WORDCOUNT",
    pages.filter((p) => (p.extract?.mainWordCount ?? 0) < floor).map((p) => p.url),
    { floor, median },
  );
  push(
    out,
    "THIN.NO_MAIN_CONTENT",
    pages.filter((p) => (p.extract?.mainWordCount ?? 0) < 50).map((p) => p.url),
    {},
  );
  push(
    out,
    "THIN.TEMPLATE_ONLY",
    pages.filter((p) => (p.extract?.boilerplateRatio ?? 0) > 0.9).map((p) => p.url),
    {},
  );
  push(
    out,
    "THIN.LOREM_IPSUM",
    pages.filter((p) => p.extract?.loremIpsum).map((p) => p.url),
    {},
  );
  if (hasReq(ctx, "gsc")) {
    push(
      out,
      "THIN.ZERO_IMPRESSIONS",
      pages
...
export function detectJs(ctx: AuditContext): FindingDraft[] {
  ...
  push(
    out,
    "JS.META_DESC_UPDATED",
    rendered
      .filter(
        (p) =>
          p.extract?.metaDescription &&
  ...
  push(
    ...
    rendered
      .filter(
        (p) =>
          (p.extract?.h1.length ?? 0) === 0 && (p.renderedExtract?.h1.length ?? 0) > 0,
      )
      .map((p) => p.url),
    {},
  );
  ...
  push(
    ...
    pages
      .filter(
        (p) => (p.extract?.links.length ?? 0) === 0 && /onclick=/i.test(p.html ?? ""),
      )
      .map((p) => p.url),
    {},
  );
  push(
    out,
    "JS.HASH_ROUTING",
    pages.filter((p) => p.url.includes("#/")).map((p) => p.url),
    {},
  );
  push(
    out,
  ...
  push(
    out,
    "JS.RENDER_TIMEOUT",
    pages.filter((p) => p.error === "render_timeout").map((p) => p.url),
    {},
  );
  push(
    out,
    "JS.JSONLD_RENDER_ONLY",
    rendered
      .filter(
        (p) =>
...
export function detectPerf(ctx: AuditContext): FindingDraft[] {
  ...
  if (hasReq(ctx, "crux") && ctx.crux?.ttfbP75Ms && ctx.crux.ttfbP75Ms > 800) {
    ...
    push(
      out,
      "PERF.TTFB",
      pages.filter((p) => p.ttfbMs > 800).map((p) => p.url),
      {},
    );
  }
  ...
  push(
    out,
    "PERF.NO_CACHE_HEADERS",
    pages
      .filter(
        (p) =>
          /\.(js|css|woff2|png|jpe?g|webp|avif)(\?|$)/i.test(p.url) &&
  ...
  push(
    ...
    pages
      .filter(
        (p) =>
          /html|javascript|json|css|text\//i.test(p.contentType) &&
  ...
  push(
    ...
    pages
      .filter(
        (p) =>
          p.depth === 0 &&
          !/(cf-ray|x-amz-cf-id|x-vercel-cache|x-cache|fastly)/i.test(
            Object.keys(p.headers).join(" "),
          ),
      )
      .map((p) => p.url),
    {},
  ...
  push(
    out,
    "PERF.FONT_NO_DISPLAY_SWAP",
    htmlPages(ctx)
      .filter(
        (p) => /@font-face/i.test(p.html ?? "") && !/font-display/i.test(p.html ?? ""),
      )
      .map((p) => p.url),
    {},
  );
  push(
    out,
    "PERF.PAGE_WEIGHT",
    pages.filter((p) => p.wireBytes > 3 * 1024 * 1024).map((p) => p.url),
    {},
  );
  return out;
}
...
export function detectSec(ctx: AuditContext): FindingDraft[] {
  ...
  push(
    out,
    "SEC.HTTP_URLS",
    pages.filter((p) => p.isInternal && p.url.startsWith("http:")).map((p) => p.url),
    {},
  );
  if (
    ctx.crawl.originProbe.https &&
  ...
  if (
    ctx.crawl.originProbe.certDaysRemaining !== null &&
  ...
  const hsts =
    home?.headers["strict-transport-security"] ?? ctx.crawl.originProbe.hsts ?? "";
  ...
  push(
    ...
    htmlPages(ctx)
      .filter(
        (p) =>
          p.url.startsWith("https:") && (absCanonical(p) ?? "").startsWith("http:"),
      )
      .map((p) => p.url),
    {},
  );
...
export function detectHref(ctx: AuditContext): FindingDraft[] {
  ...
  const map = byUrl(ctx);
  const iso639 =
    /^(?:[a-z]{2,3})(?:-[A-Z][a-z]{3})?(?:-[A-Z]{2}|-[0-9]{3})?$|^x-default$/;
  push(
    out,
    "HREF.MISSING_SELF",
    pages
      .filter(
        (p) =>
          !p.extract?.hreflang.some(
            (h) => stripSlash(h.absUrl) === stripSlash(p.finalUrl),
          ),
      )
      .map((p) => p.url),
    {},
  ...
  push(
    out,
    "HREF.INVALID_LANG_CODE",
    pages
      .filter((p) =>
        p.extract?.hreflang.some(
          (h) => h.lang !== "x-default" && !/^[a-z]{2,3}/i.test(h.lang),
        ),
      )
      .map((p) => p.url),
    {},
  ...
  push(
    ...
    pages
      .filter((p) =>
        p.extract?.hreflang.some((h) => !iso639.test(h.lang) && h.lang !== "x-default"),
      )
      .map((p) => p.url),
    {},
  );
  ...
  push(
    out,
    "HREF.OUTSIDE_HEAD",
    pages.filter((p) => p.extract?.hreflang.some((h) => !h.inHead)).map((p) => p.url),
    {},
  );
  push(
    out,
    "HREF.MISSING_X_DEFAULT",
    pages
  ...
  push(
    out,
    "HREF.MULTIPLE_X_DEFAULT",
    pages
      .filter(
        (p) =>
          (p.extract?.hreflang.filter((h) => h.lang === "x-default").length ?? 0) > 1,
      )
      .map((p) => p.url),
    {},
  );
  push(
    out,
    "HREF.UNLINKED_ALTERNATE",
    pages
      .filter((p) =>
        p.extract?.hreflang.some((h) => (map.get(h.absUrl)?.inlinkCount ?? 0) === 0),
      )
      .map((p) => p.url),
    {},
  );
  ...
  push(
    out,
    "HREF.HTML_LANG_MISMATCH",
    pages
      .filter((p) => {
        const self = p.extract?.hreflang.find(
          (h) => stripSlash(h.absUrl) === stripSlash(p.finalUrl),
        );
        return Boolean(
          self &&
...
export function detectSd(ctx: AuditContext): FindingDraft[] {
  ...
  for (const p of pages) {
    ...
    push(
      out,
      "SD.INVALID_URL_PROP",
      (p.extract?.jsonLd ?? []).some((b) =>
        JSON.stringify(b.parsed).includes('"url":"/"'),
      )
        ? [p.url]
        : [],
      {},
  ...
  push(
    ...
    pages
      .filter(
        (p) =>
          JSON.stringify(p.extract?.jsonLd ?? "").includes("aggregateRating") &&
  ...
  push(
    out,
    "SD.MISSING_ON_ELIGIBLE_TEMPLATE",
    pages
      .filter((p) => {
        const path = originPath(p.url);
        const types = (p.extract?.jsonLd ?? []).flatMap((b) => flattenTypes(b.parsed));
        return (
          /\/(blog|article|post)\//i.test(path) && !types.some((t) => /Article/.test(t))
        );
      })
      .map((p) => p.url),
    {},
...
export function detectOnp(ctx: AuditContext): FindingDraft[] {
  ...
  push(
    out,
    "ONP.TITLE_MISSING",
    pages.filter((p) => !p.extract?.title).map((p) => p.url),
    {},
  );
  push(
    out,
    "ONP.TITLE_MULTIPLE",
    pages.filter((p) => (p.extract?.titles.length ?? 0) > 1).map((p) => p.url),
    {},
  );
  push(
    out,
    "ONP.TITLE_OUTSIDE_HEAD",
    pages.filter((p) => p.extract?.title && !p.extract.titleInHead).map((p) => p.url),
    {},
  );
  push(
    out,
  ...
  push(
    out,
    "ONP.TITLE_TOO_LONG",
    pages.filter((p) => (p.extract?.title?.length ?? 0) > 60).map((p) => p.url),
    {},
  );
  push(
    out,
  ...
  push(
    ...
    pages.filter((p) => (p.extract?.title?.length ?? 0) > 70).map((p) => p.url),
    {},
  );
  push(
    out,
    "ONP.TITLE_SAME_AS_H1",
    pages
  ...
  push(
    ...
    pages.filter((p) => !p.extract?.metaDescription).map((p) => p.url),
    {},
  );
  push(
    out,
    "ONP.META_DESC_MULTIPLE",
    pages
  ...
  push(
    out,
    "ONP.H1_MISSING",
    pages.filter((p) => (p.extract?.h1.length ?? 0) === 0).map((p) => p.url),
    {},
  );
  push(
    out,
    "ONP.H1_MULTIPLE",
    pages.filter((p) => (p.extract?.h1.length ?? 0) > 1).map((p) => p.url),
    {},
  );
  push(
    out,
  ...
  push(
    out,
    "ONP.H1_TOO_LONG",
    pages.filter((p) => (p.extract?.h1[0]?.length ?? 0) > 70).map((p) => p.url),
    {},
  );
  push(
    out,
  ...
  push(
    out,
    "ONP.NO_VIEWPORT",
    pages.filter((p) => !p.extract?.viewport).map((p) => p.url),
    {},
  );
  push(
    out,
    "ONP.VIEWPORT_FIXED_WIDTH",
    pages
      .filter((p) =>
        /width\s*=\s*1024|user-scalable\s*=\s*no/i.test(p.extract?.viewport ?? ""),
      )
      .map((p) => p.url),
    {},
  );
  push(
    out,
    "ONP.NO_LANG",
    pages.filter((p) => !p.extract?.lang).map((p) => p.url),
    {},
  );
  push(
    out,
    "ONP.NO_CHARSET",
    pages.filter((p) => !p.extract?.charsetInFirst1024).map((p) => p.url),
    {},
  );
  ...
  push(
    out,
    "ONP.URL_OVER_115_CHARS",
    pages.filter((p) => p.url.length > 115).map((p) => p.url),
    {},
  );
  push(
    out,
    "ONP.URL_UPPERCASE",
    pages.filter((p) => /[A-Z]/.test(originPath(p.url))).map((p) => p.url),
    {},
  );
  push(
    out,
    "ONP.URL_UNDERSCORES",
    pages.filter((p) => originPath(p.url).includes("_")).map((p) => p.url),
    {},
  );
  push(
    out,
    "ONP.URL_SPACES",
    pages.filter((p) => p.url.includes("%20") || p.url.includes(" ")).map((p) => p.url),
    {},
  );
  push(
    out,
  ...
  push(
    out,
    "ONP.URL_MULTIPLE_SLASHES",
    pages.filter((p) => originPath(p.url).includes("//")).map((p) => p.url),
    {},
  );
  push(
    out,
    "ONP.URL_REPETITIVE_PATH",
    pages
      .filter((p) => {
        const segs = originPath(p.url).split("/").filter(Boolean);
...
export function detectImg(ctx: AuditContext): FindingDraft[] {
  ...
  push(
    ...
    pages.filter((p) => p.extract?.images.some((i) => !i.hasAltAttr)).map((p) => p.url),
    {},
  );
  push(
    out,
    "IMG.MISSING_ALT_TEXT",
    pages
  ...
  push(
    out,
    "IMG.ALT_KEYWORD_STUFFED",
    pages
      .filter((p) =>
        p.extract?.images.some((i) => {
  ...
  push(
    out,
    "IMG.NO_SRCSET",
    pages.filter((p) => p.extract?.images.some((i) => !i.srcset)).map((p) => p.url),
    {},
  );
  push(
    out,
    "IMG.LCP_LAZY",
    pages.filter((p) => p.extract?.images[0]?.loading === "lazy").map((p) => p.url),
    {},
  );
  push(
    out,
    "IMG.BELOW_FOLD_EAGER",
    pages
...
export function detectMob(ctx: AuditContext): FindingDraft[] {
  ...
  push(
    out,
    "MOB.UNSUPPORTED_PLUGIN",
    pages.filter((p) => p.extract?.plugins).map((p) => p.url),
    {},
  );
  ...
  push(
    out,
    "MOB.SEPARATE_M_DOT",
    pages.filter((p) => /^m\./i.test(new URL(p.url).hostname)).map((p) => p.url),
    {},
  );
  push(
    out,
...
export function detectConf(ctx: AuditContext): FindingDraft[] {
  ...
  push(
    out,
    "CONF.NOINDEX_IN_SITEMAP",
    pages.filter((p) => noindexOf(p) && sm.has(p.url)).map((p) => p.url),
    {},
  );
  push(
    out,
    "CONF.DISALLOW_PLUS_NOINDEX",
    pages.filter((p) => !p.robotsAllowed && noindexOf(p)).map((p) => p.url),
    {},
  );
  push(
    out,
    "CONF.CANON_VS_HREFLANG",
    pages
      .filter((p) => {
        const c = absCanonical(p);
  ...
  push(
    out,
    "CONF.CANON_VS_REDIRECT",
    pages.filter((p) => hops(p) >= 1 && absCanonical(p)).map((p) => p.url),
    {},
  );
  if (hasReq(ctx, "gsc") && ctx.gsc?.googleCanonicalByUrl) {
    push(
      out,
      "CONF.GOOGLE_CHOSE_OTHER",
      Object.entries(ctx.gsc.googleCanonicalByUrl)
        .filter(([u, g]) => {
  ...
  push(
    out,
    "CONF.SITEMAP_VS_NOINDEX_HEADER",
    pages
      .filter(
        (p) =>
          sm.has(p.url) &&
  ...
  push(
    out,
    "CONF.PAGINATED_CANON_TO_P1",
    pages
      .filter(
        (p) => /[?&]page=\d+/i.test(p.url) && /page=1/i.test(absCanonical(p) ?? ""),
      )
      .map((p) => p.url),
    {},
  );
...
export function detectGap(ctx: AuditContext): FindingDraft[] {
  ...
  push(
    out,
    "GAP.ORPHAN_SITEMAP",
    pages.filter((p) => p.inSitemap && p.followInlinkCount === 0).map((p) => p.url),
    {},
  );
  if (hasReq(ctx, "gsc")) {
...
export function detectSoft404(ctx: AuditContext): FindingDraft[] {
  const out: FindingDraft[] = [];
  const phrase =
    /\b(page not found|not found|404|no results|nothing found|this page (doesn't|does not) exist|error 404|no longer available|coming soon|under construction)\b/i;
  if (ctx.crawl.originProbe.randomSoft404) {
    push(
      out,
      "SOFT404.RANDOM_URL_200",
      [ctx.crawl.originProbe.randomSoft404Url ?? ctx.crawl.origin],
      {},
    );
  }
  push(
  ...
  push(
    out,
    "SOFT404.REDIRECT_TO_HOME",
    ctx.crawl.pages
      .filter(
        (p) =>
          hops(p) >= 1 && originPath(p.finalUrl) === "/" && originPath(p.url) !== "/",
      )
      .map((p) => p.url),
    {},
  );
  push(
    ...
    htmlPages(ctx)
      .filter(
        (p) =>
          /out of stock|sold out/i.test(p.extract?.allText ?? "") &&
