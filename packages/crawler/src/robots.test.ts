...
import { isAllowedByRobots, matchRule, parseRobotsTxt, robotsFromFetch } from "./robots.js";
...
Allow: /secret/public
`);
    const robots = { ...parsed, statusCode: 200, contentType: "text/plain", bytes: 10, redirectHops: 0, error: null, mode: "parsed" as const };
...
`);
  ...
  it("4xx robots is allow-all; 5xx is disallow-all", () => {
    const allow = robotsFromFetch({ statusCode: 404, raw: "", contentType: "text/plain", bytes: 0, redirectHops: 0, error: null });
    expect(allow.mode).toBe("allow-all");
    expect(isAllowedByRobots(allow, "https://ex.com/x")).toBe(true);
    const deny = robotsFromFetch({ statusCode: 503, raw: "", contentType: "text/plain", bytes: 0, redirectHops: 0, error: null });
