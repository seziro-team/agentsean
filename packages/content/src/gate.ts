...
function nearDup(ctx: GateContext): GateCheck {
  ...
  if (self && ctx.kind === "refresh") {
    ...
    if (
      nearDuplicate(hex, simhashHex(self.body), 1) &&
      ...
      return check(
        ...
        "NEAR_DUPLICATE",
        false,
        "draft is effectively the existing page",
      );
    }
  }
...
function fleschEase(text: string): number {
  ...
  const syllables = words.reduce(
    (n, w) => n + Math.max(1, (w.match(/[aeiouy]+/gi) ?? []).length),
    0,
  );
...
function readability(ctx: GateContext): GateCheck {
  ...
  const h2 =
    (ctx.draft.body.match(/^##\s+/gm) ?? []).length +
  ...
  if (ease < 20)
    return check(
      ...
      "READABILITY",
      false,
      `Flesch ease ${ease.toFixed(0)} is unreadable`,
    );
  return check(
    3,
    "READABILITY",
    true,
    `words=${words} h2=${h2} flesch=${ease.toFixed(0)}`,
  );
}
  ...
  for (const [preferred, synonym] of Object.entries(ctx.style.preferredTerms)) {
    if (
      ...
      return check(
        ...
        "BRAND_VOICE",
        false,
        `use "${preferred}" instead of "${synonym}"`,
      );
    }
  }
...
function rateLimit(ctx: GateContext): GateCheck {
  if (ctx.kind === "create" && ctx.newPagesToday >= BLAST.newPagesPerDay) {
    return check(
      ...
      "RATE_LIMIT",
      false,
      `new-page cap is ${BLAST.newPagesPerDay}/day/site`,
    );
  }
  return check(
    8,
    "RATE_LIMIT",
    true,
    `new pages today ${ctx.newPagesToday}/${BLAST.newPagesPerDay}`,
  );
}
...
function vertical(ctx: GateContext): GateCheck {
  ...
  if (blocked) {
    return check(
      ...
      "VERTICAL_BLOCK",
      false,
      `content generation is T4-blocked for ${cat} sites`,
    );
  }
