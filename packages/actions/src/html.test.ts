...
describe("live HTML title", () => {
  it("extracts and patches <title>", () => {
    ...
    expect(htmlTitle(patchHtmlTitle(html, "New Title For The Page"))).toBe(
  ...
  it("verifies by re-fetching live HTML, not the write-API status", async () => {
    ...
    const fetchFn = (async () =>
      new Response(live.html, { status: 200 })) as typeof fetch;
    const miss = await verifyLiveTitle(
      "https://example.com/",
      "New Title For The Page",
      fetchFn,
    );
    ...
    const hit = await verifyLiveTitle(
      "https://example.com/",
      "New Title For The Page",
      fetchFn,
    );
