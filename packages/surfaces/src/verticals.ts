...
export type VerticalSignal = { id: string; hit: boolean; pointsTo: VerticalPreset; weight: number };
...
export const ONBOARDING_QUESTIONS = [
  { id: "win", prompt: "What does a win look like?", options: ["signups/trials", "demo requests or quote forms", "phone calls or bookings", "ad revenue / pageviews", "subscriptions", "listings posted or applications", "affiliate clicks/commissions", "transactions on the platform"] },
  { id: "indexable", prompt: "Roughly how many pages should be in Google?", options: ["<100", "100–2k", "2k–50k", "50k–1M", ">1M"] },
  { id: "velocity", prompt: "Do you publish new content more than 3× per week?", options: ["yes", "no"] },
  { id: "third_party", prompt: "Is any of your content written by people who don't work for you?", options: ["yes", "no", "some"] },
  { id: "regulated", prompt: "Are you in a regulated space?", options: ["health/medical", "finance/insurance", "legal", "none"] },
  { id: "geo", prompt: "Where do you serve customers?", options: ["one location", "multiple named locations", "a region/country", "globally, online only"] },
] as const;

const SAAS_SUPPRESS = 41;

function signal(id: string, hit: boolean, pointsTo: VerticalPreset, weight = 1): VerticalSignal {
...
export function scoreSignals(input: VerticalInput): VerticalSignal[] {
  ...
  return [
    ...
    signal("S4", input.jsonLdTypes.filter((t) => /NewsArticle/i.test(t)).length > 0, "publisher_news"),
    ...
    signal("S10", input.jsonLdTypes.some((t) => /DiscussionForumPosting|QAPage/i.test(t)), "marketplace_ugc"),
    signal("S11", input.hasPricing && input.hasDemoOrSignup, "b2b_saas", 3),
    signal("S12", urls.some((u) => /\/(vs|alternatives|compare|integrations)\b/.test(u)), "b2b_saas", 2),
    signal("S13", input.urls.length < 2000 && input.hasPricing && !input.productSellerVaries, "b2b_saas", 2),
    signal("S14", input.telLinkShare > 0.6 && !input.hasPricing, "b2b_lead_gen", 2),
    signal("S15", urls.some((u) => /\/(locations|store-locator|branches|find-a)\b/.test(u)), "multi_location", 3),
    signal("S16", input.localityCount >= 5, "multi_location", 2),
    signal("S17", input.outboundSponsoredShare > 0.15, "affiliate_review", 3),
    signal("S18", input.titles.filter((t) => /\bbest |\breview\b|\b vs \b|\btop \d/i.test(t)).length / Math.max(input.titles.length, 1) > 0.3, "affiliate_review", 2),
    signal("S19", input.jsonLdTypes.some((t) => t === "Paywalled"), "publisher_news"),
    signal("S20", input.jsonLdTypes.some((t) => /MedicalWebPage|Physician|MedicalClinic/i.test(t)) || input.medicalLexiconShare > 0.1, "ymyl", 3),
    signal("S21", urls.filter((u) => /\/(homes|property|for-sale)\b/.test(u)).length > 1000, "jobs_classifieds"),
    signal("S22", input.jsonLdTypes.some((t) => /Vehicle|Car/i.test(t)), "jobs_classifieds"),
...
export function verticalRules(preset: VerticalPreset): string[] {
  switch (preset) {
    case "b2b_saas":
      return ["Add bottom-funnel pages (/vs, /alternatives, /integrations).", "Refresh decaying blog posts before minting URLs.", "Git-backed execution as a PR."];
    case "marketplace_ugc":
      return ["Stop indexing thin template pages.", "Faceted nav: robots.txt disallow is preferred (Google 2025-12-18)."];
    case "publisher_news":
      return ["Split Discover vs web alerts.", "Do not generate scaled news URLs."];
    case "affiliate_review":
      return ["Content generation is T4 hard-blocked.", "Thin affiliation is a spam policy."];
    case "ymyl":
      return ["Content generation is T4 hard-blocked.", "Human review required for any medical/finance/legal claim."];
    case "multi_location":
      return ["City×service pages are T4 unless unique data per URL.", "GBP hours/categories are the safe writes."];
    case "b2b_lead_gen":
