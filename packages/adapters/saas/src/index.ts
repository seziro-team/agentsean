...
export function createWebflowAdapter(opts: TokenOpts & { pageId?: string | undefined }): SiteAdapter {
  return jsonWrite(
    "webflow",
    opts,
    (url) => {
...
export function createGhostAdapter(opts: {
  ...
  const adapter = createHttpTitleAdapter({
    ...
    write: async (url, title) => {
      ...
      const res = await fetchFn(`${opts.origin.replace(/\/+$/, "")}/ghost/api/admin/posts/?filter=url:${encodeURIComponent(url)}`, {
        method: "PUT",
        headers: {
...
export function createWixAdapter(opts: TokenOpts & { siteId?: string | undefined }): SiteAdapter {
  const fetchFn = opts.fetch ?? fetch;
  return createHttpTitleAdapter({
    kind: "wix",
    fetch: fetchFn,
...
export function createBigCommerceAdapter(opts: {
  ...
  return createHttpTitleAdapter({
    ...
    fetch: fetchFn,
    write: async (url, title) => {
      const res = await fetchFn(`https://api.bigcommerce.com/stores/${opts.storeHash}/v2/pages`, {
        method: "PUT",
        headers: {
